...
void ImageAvatarLite::initSprites(bool is_change) {
    ...
    params_eye_s p_eye_r = _config.getEyeRightParameters(_expression);
    params_eye_s p_eye_l = _config.getEyeLeftParameters(_expression);
    // 固定パーツ
    _eye_r_op_sp->setPsram(_spcommon.psram);
    _eye_r_op_sp->setColorDepth(_spcommon.color_depth);
    _eye_r_op_sp->setSwapBytes(_spcommon.swap_bytes);
    _eye_r_op_sp->createFromBmpFile(*_bmp_fs, p_eye_r.filename_op);
    _eye_r_cl_sp->setPsram(_spcommon.psram);
    _eye_r_cl_sp->setColorDepth(_spcommon.color_depth);
    _eye_r_cl_sp->setSwapBytes(_spcommon.swap_bytes);
    _eye_r_cl_sp->createFromBmpFile(*_bmp_fs, p_eye_r.filename_cl);

    _eye_l_op_sp->setPsram(_spcommon.psram);
    _eye_l_op_sp->setColorDepth(_spcommon.color_depth);
    _eye_l_op_sp->setSwapBytes(_spcommon.swap_bytes);
    _eye_l_op_sp->createFromBmpFile(*_bmp_fs, p_eye_l.filename_op);
    _eye_l_cl_sp->setPsram(_spcommon.psram);
    _eye_l_cl_sp->setColorDepth(_spcommon.color_depth);
    _eye_l_cl_sp->setSwapBytes(_spcommon.swap_bytes);
    _eye_l_cl_sp->createFromBmpFile(*_bmp_fs, p_eye_l.filename_cl);
    ...
    _eye_l_sp->createSprite(p_eye_l.picinfo.w, p_eye_l.picinfo.h);
    ...
    _eye_r_sp->createSprite(p_eye_r.picinfo.w, p_eye_r.picinfo.h);
...
void ImageAvatarLite::drawEyes() {
    ...
    params_eye_s p_eye_r = _config.getEyeRightParameters(_expression);
    // 左目の描画
    if (_mv.eye_l_ratio < p_eye_l.min_scaleY) {
        _eye_l_cl_sp->pushRotateZoom(_eye_l_sp,
            p_eye_l.picinfo.w / 2, p_eye_l.picinfo.h / 2,
            0, 1.0, 1.0);
    } else {
        _eye_l_op_sp->pushRotateZoom(_eye_l_sp,
            p_eye_l.picinfo.w / 2  , p_eye_l.picinfo.h / 2,
            0, 1.0, _mv.eye_l_ratio);
    }
    _eye_l_sp->pushRotateZoom(_lcd_sp,
                             p_eye_l.picinfo.x, p_eye_l.picinfo.y + _mv.breath,
                             0, 1.0, 1.0, _tp_color);

    // 右目の描画
    if (_mv.eye_r_ratio < p_eye_r.min_scaleY) {
        _eye_r_cl_sp->pushRotateZoom(_eye_r_sp,
            p_eye_r.picinfo.w / 2, p_eye_r.picinfo.h / 2,
            0, 1.0, 1.0);
    } else {
        _eye_r_op_sp->pushRotateZoom(_eye_r_sp,
            p_eye_r.picinfo.w / 2, p_eye_r.picinfo.h / 2,
            0, 1.0, _mv.eye_r_ratio);

    }
    _eye_r_sp->pushRotateZoom(_lcd_sp,
                             p_eye_r.picinfo.x, p_eye_r.picinfo.y + _mv.breath,
                             0, 1.0, 1.0, _tp_color);
}
...
void ImageAvatarLite::deleteSprites() {
    ...
    _eye_r_op_sp->deleteSprite();
    _eye_r_cl_sp->deleteSprite();
    _eye_l_op_sp->deleteSprite();
    _eye_l_cl_sp->deleteSprite();
