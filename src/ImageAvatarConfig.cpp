...
void ImageAvatarConfig::setEyesParameters(DynamicJsonDocument doc) {
    for (int i=0; i<_max_expresssion; i++) {
        JsonObject elem = doc["eye_r"][i][_expression[i]];
        if (!elem.isNull()){
            _eye_r[i].picinfo.x = elem["x"];
            _eye_r[i].picinfo.y = elem["y"];
            _eye_r[i].picinfo.w = elem["w"];
            _eye_r[i].picinfo.h = elem["h"];
            _eye_r[i].filename_op = elem["filename"]["open"];
            _eye_r[i].filename_cl = elem["filename"]["close"];
            _eye_r[i].min_scaleX = elem["minScaleX"];
            _eye_r[i].max_scaleX = elem["maxScaleX"];
            _eye_r[i].min_scaleY = elem["minScaleY"];
            _eye_r[i].max_scaleY = elem["maxScaleY"];
        }
        elem = doc["eye_l"][i][_expression[i]];
        if (!elem.isNull()){
            _eye_l[i].picinfo.x = elem["x"];
            _eye_l[i].picinfo.y = elem["y"];
            _eye_l[i].picinfo.w = elem["w"];
            _eye_l[i].picinfo.h = elem["h"];
            _eye_l[i].filename_op = elem["filename"]["open"];
            _eye_l[i].filename_cl = elem["filename"]["close"];
            _eye_l[i].min_scaleX = elem["minScaleX"];
            _eye_l[i].max_scaleX = elem["maxScaleX"];
            _eye_l[i].min_scaleY = elem["minScaleY"];
            _eye_l[i].max_scaleY = elem["maxScaleY"];
        }
    }
}
...
params_eye_s ImageAvatarConfig::getEyeRightParameters(uint8_t expression) {
    return _eye_r[expression];
}

params_eye_s ImageAvatarConfig::getEyeLeftParameters(uint8_t expression) {
    return _eye_l[expression];
}
...
void ImageAvatarConfig::printAllParameters() {
    ...
    for (int i=0; i<_max_expresssion; i++) {
        ...
        Serial.println(_eye_r[i].filename_op);
        Serial.println(_eye_r[i].filename_cl);
        Serial.println(_eye_l[i].filename_op);
        Serial.println(_eye_l[i].filename_cl);
